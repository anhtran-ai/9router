...
export async function GET(request, { params }) {
  try {
    ...
    if (kindFilter) {
      const data = await buildModelsList(kindFilter, { signal: request?.signal });
    ...
    const models = await buildModelsList([LLM_KIND], { signal: request?.signal });
    ...
    // Model discovery can involve live provider calls. Do not reflect or log
    // provider exception text because upstreams may echo authorization data.

...
import { awaitWithSignal, throwIfAborted } from "open-sse/utils/abort.js";
...
export async function handleAntigravityQuotaError(connectionId, status, model, accessToken, providerSpecificData, signal = null) {
  ...
  const quota = (await awaitWithSignal(
    refreshAntigravityQuota(connectionId, accessToken, providerSpecificData),
    signal,
  ))?.[model];
  // The shared refresh may finish in the background, but an abandoned request
  // must never add strikes or route to another account after cancellation.

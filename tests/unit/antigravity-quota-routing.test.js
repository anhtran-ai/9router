...
describe("Antigravity quota-aware routing", () => {
  ...
  it("abandons a cancelled quota wait without adding a strike", async () => {
    let resolveUsage;
    mocks.getAntigravityUsage.mockReturnValue(new Promise(resolve => { resolveUsage = resolve; }));
    const controller = new AbortController();

    const pending = handleAntigravityQuotaError(
      "ag-cancelled", 429, MODEL, "token", {}, controller.signal,
    );
    await vi.waitFor(() => expect(mocks.getAntigravityUsage).toHaveBeenCalledTimes(1));
    controller.abort();
    await expect(pending).rejects.toMatchObject({ name: "AbortError" });

    resolveUsage({ quotas: { [MODEL]: { remainingPercentage: 90, resetAt: FUTURE_RESET } } });
    await refreshAntigravityQuota("ag-cancelled", "token", {});

    // If the cancelled request had recorded strike #1, the second call below
    // would become strike #3 and return a synthetic 15-minute block.
    await expect(handleAntigravityQuotaError("ag-cancelled", 429, MODEL, "token", {})).resolves.toBeNull();
    await expect(handleAntigravityQuotaError("ag-cancelled", 429, MODEL, "token", {})).resolves.toBeNull();
    expect(getAntigravityQuotaCache().get("ag-cancelled")?.[MODEL]?.remainingPercentage).toBe(90);
  });
